...
THIS_YEAR = 2026
# 발간일이 원문으로 확인된 판만. 나머지는 모르는 채로 둔다.
PUBLISHED = {'2024': '2025년 8월'}
...
def from_whitepaper(ex):
    ...
    for it in j['items']:
        ...
        if it['hidden']:
            ...
            # 백서 이름의 연도는 '수록 연도'다. 실제 발간은 이듬해다 — 2024년판은 2025년 8월에
            # 나왔다. 방은 이름의 연도에 두되(그 해의 기록이므로), 발간이 이듬해임을 라벨에 못 박는다.
            # 발간일이 확인된 판만 적는다. 나머지는 '미확인'으로 두고 채우지 않는다.
            pub = PUBLISHED.get(span)
            add(ex, y=y, date=f'{span} 수록 · 발간 {pub}' if pub else f'{span} 수록 · 발간일 미확인',
                theme='record', w=66, tag='백서',
                title=f'{span}년 {m.group(3) or "백서"}' + (f' — {pub} 발간' if pub else ''),
                body=(f'{span}년의 활동을 질병관리청이 스스로 정리한 공식 기록. '
                      + (f'{pub}에 나왔다. ' if pub else '발간일은 원문 표지에서 확인해야 한다. ')
                      + '백서는 그 해가 끝난 뒤에 쓰이므로 이름의 연도와 발간 연도가 다르다.'),
                link=it['file'] or '../whitepaper/', art=art,
                why=('이 액자의 연도는 백서 이름의 연도, 즉 무엇을 기록했는가의 연도다. 실제로 세상에 나온 것은 '
                     '이듬해다(2024년판 → 2025년 8월). 그래서 올해 방에는 백서가 걸릴 수 없다 — '
                     f'{THIS_YEAR}년의 백서는 {THIS_YEAR + 1}년에 나온다. '
                     '백서의 "향후 추진계획"은 다음 해 제도 변화의 예고편이기도 하다.'))
